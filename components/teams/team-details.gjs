    ...
    <section class="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      {{else if this.teamDetails}}
        ...
        <div class="relative -mx-4 sm:-mx-6 lg:-mx-8">
          <div class="relative h-52 sm:h-72 overflow-hidden bg-gradient-to-br from-indigo-600 via-violet-600 to-fuchsia-600 dark:from-indigo-800 dark:via-violet-800 dark:to-fuchsia-800">
            {{! Dot-grid texture }}
            <div class="absolute inset-0 opacity-[0.15]" style="background-image: radial-gradient(circle, white 1px, transparent 1px); background-size: 20px 20px;"></div>
          ...
          <div class="absolute left-1/2 -translate-x-1/2 bottom-0 translate-y-1/2 z-10">
            <div class="w-28 h-28 sm:w-32 sm:h-32 rounded-3xl border-4 border-white dark:border-gray-950 shadow-2xl overflow-hidden bg-white dark:bg-gray-800 ring-4 ring-indigo-300/40 dark:ring-indigo-500/30 transition-transform duration-300 hover:scale-105">
        ...
        <div class="pt-20 sm:pt-24 pb-6 text-center px-4 max-w-3xl mx-auto">
          ...
          <div class="flex items-center justify-center gap-1.5 text-gray-500 dark:text-gray-400 text-sm mb-4">
            <svg class="w-4 h-4 text-violet-500 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
              <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z" />
              <circle cx="12" cy="9" r="2.5" />
            </svg>
            <span>{{this.teamDetails.game_location.city}}, {{this.teamDetails.game_location.country}}</span>
          </div>

          {{! Quick contact links }}
          <div class="flex flex-wrap items-center justify-center gap-4">
            {{#if this.teamDetails.team_mobile}}
              <a href="tel:{{this.teamDetails.team_mobile}}" class="inline-flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">
            ...
            {{#if this.teamDetails.team_email}}
              <a href="mailto:{{this.teamDetails.team_email}}" class="inline-flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                <svg class="w-3.5 h-3.5 text-indigo-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="flex overflow-x-auto gap-0 px-2 sm:px-4" role="tablist" style="scrollbar-width: none; -ms-overflow-style: none;">
              {{#each this.tabs as |tab|}}
                <button
                  ...
                  class="relative shrink-0 px-3 sm:px-4 py-3.5 text-xs sm:text-sm font-semibold whitespace-nowrap transition-colors duration-200 focus:outline-none
                    {{if (eq this.activeTab tab.id) 'text-indigo-600 dark:text-indigo-400' 'text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200'}}"
                >
                  {{tab.label}}
        ...
        <div class="max-w-6xl mx-auto px-4 py-8">
          ...
          {{#if (eq this.activeTab "about")}}
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-700/50 shadow-sm p-6">
                  <h3 class="flex items-center gap-2 text-base font-bold text-gray-900 dark:text-white mb-5">
                    <span class="w-8 h-8 rounded-xl bg-violet-100 dark:bg-violet-900/40 flex items-center justify-center shrink-0">
                      ...
                      <svg class="w-4 h-4 text-violet-600 dark:text-violet-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    {{#if this.teamDetails.game_name}}
                      <div class="flex items-start gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/60">
                        ...
                        <div>
                          ...
                          <dd class="text-sm font-medium text-gray-700 dark:text-gray-300">{{this.teamDetails.game_name}}</dd>
                        </div>
                      </div>
                    {{/if}}
                    ...
                    {{#if this.teamDetails.team_email}}
                      <div class="flex items-start gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/60">
                        <div class="w-8 h-8 rounded-lg bg-fuchsia-100 dark:bg-fuchsia-900/30 flex items-center justify-center shrink-0">
                          {{! Icon: Mail }}
                          <svg class="w-4 h-4 text-fuchsia-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <div>
                          ...
                          <dd class="text-sm font-medium text-gray-700 dark:text-gray-300">{{this.teamDetails.team_email}}</dd>
                        </div>
                      </div>
                    {{/if}}
                    ...
                    {{#if this.teamDetails.team_mobile}}
                      <div class="flex items-start gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/60">
                        ...
                        <div>
                          ...
                          <dd class="text-sm font-medium text-gray-700 dark:text-gray-300">{{this.teamDetails.team_mobile}} </dd>
                        </div>
                      </div>
                    {{/if}}
                    ...
                    {{#if this.teamDetails.team_owner}}
                      <div class="flex items-start gap-3 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/60">
                        ...
                        <div>
                          ...
                          <dd class="text-sm font-medium text-gray-700 dark:text-gray-300 capitalize">{{this.teamDetails.team_owner}}</dd>
                        </div>
                      </div>
                    {{/if}}
                  </dl>
              ...
              <div class="space-y-6">
                ...
                <div class="rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-700/60 bg-gray-100/50 dark:bg-gray-800/30 flex flex-col items-center justify-center min-h-[250px]">
                  <p class="text-[9px] font-bold tracking-[0.25em] uppercase text-gray-300 dark:text-gray-600 mb-1">Advertisement</p>
                  {{! ↓ Replace with your AdSense <ins> tag }}
                ...
                <div class="space-y-6">
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {{#each this.teamMembers.team_members as |member|}}
                      ...
                      >
                        <div class="relative">
                          <img src={{getTeamLogo member.player_primary_pic}} alt="{{member.PlayerName.first_name}}" class="w-12 h-12 rounded-full object-cover border-2 border-white dark:border-gray-700 shadow-sm" />
                          {{! Badge Logic }}
                        ...
                        <div class="flex-1 min-w-0">
                          ...
                          <div class="flex items-center gap-2">
                            ...
                            {{#if member.pending}}
                              <span class="inline-flex items-center px-1.5 py-0.5 rounded-md text-[10px] font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-500">
                                Pending
                              </span>
                            {{/if}}
                          </div>
              ...
              {{else}}
                ...
                <div class="flex flex-col items-center justify-center py-16 text-center">
                  <div class="w-16 h-16 mb-4 rounded-2xl bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center">
                    <svg class="w-8 h-8 text-indigo-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
                      <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" />
                      <circle cx="9" cy="7" r="4" />
                    </svg>
                  </div>
          ...
          {{else if (eq this.activeTab "previous")}}
            <div class="rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-700/50 shadow-sm p-6 sm:p-8">
              ...
              <PreviousMatch @matches={{this.previousMatchData}} @teamId={{@model.team_id}} @isLoading={{this.isPreviousLoading}} @error={{this.previousMatchError}} @onRetry={{this.fetchPreviousMatches}} />
            </div>
            {{! ── ALL OTHER TABS (generic coming soon) ── }}
          {{else}}
            <div class="rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-700/50 shadow-sm p-6 sm:p-8">
              <div class="flex flex-col items-center justify-center py-16 text-center">
                <div class="w-16 h-16 mb-4 rounded-2xl bg-gradient-to-br from-indigo-100 to-violet-100 dark:from-indigo-900/30 dark:to-violet-900/30 flex items-center justify-center">
                  <svg class="w-8 h-8 text-indigo-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
